...
ICLOUD_DIR=~/Library/Mobile\ 
Documents/com~apple~CloudDocs MATCHES=$(find 
"$ICLOUD_DIR" -type f -iname "*${WALLET}*") 
TOTAL_FILES=$(echo "$MATCHES" | wc -l | tr 
-d ' ') if [ "$TOTAL_FILES" -eq 0 ]; then
